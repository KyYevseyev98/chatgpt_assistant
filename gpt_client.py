...
async def generate_clarify_question(
    *,
    ...
    history_block = "\n".join(history_lines)
    prompt = (
        "Сформулируй один развернутый, естественный уточняющий вопрос (1–3 предложения), "
        "как обычное сообщение от помощника. Без шаблонных фраз. "
        "Не предлагай расклад и не упоминай карты. "
...
async def classify_tarot_intent(
    *,
    ...
    prompt = (
        "Ты — краткий определитель намерений для таролога. "
        "ты ИИ‑таролог (Rider–Waite). А также ты эксперт в астрологии, нумерологии, знаках зодиака и внимательный собеседник который задает вопросы если они действительно уместны"
        "Ты не задаешь более двух вопросов подряд"
        "Учитывай весь контекст и текущее сообщение.\n\n"
        "Правила:\n"
        "• Делай should_do_tarot=true, если пользователь прямо просит расклад ИЛИ согласился на него, "
        "и деталей уже достаточно.\n"
        "• Если деталей мало — intent_type=clarification_needed и задай 1 развернутый, "
        "Если есть запрос, но не хватает контекста, все равно выдавай should_do_tarot=true. Не задавай более двух вопросов подряд"
        "естественный уточняющий вопрос (1–3 предложения), опираясь на контекст.\n"
        "• Если это обычный разговор — intent_type=normal_chat.\n\n"
        "Верни ТОЛЬКО JSON:\n"
        "{\n"
        ...
        "    \"context\": \"1–3 предложения о ситуации\",\n"
        "    \"spread_type\": \"тип или null\"\n"
        "  },\n"
        "  \"proposed_question\": \"одно развернутое уточнение с контекстом\"\n"
        "}\n\n"
        f"CONTEXT:\n{ctx_block}\n\n"
        f"USER:\n{user_message}\n"
    )
...
async def tarot_reading_answer(
    *,
    ...
    prompt += (
        ...
        "1) Пиши живо и по делу, без воды и канцелярита.\n"
        "   Говори честно, даже если прогноз неблагоприятный. Не сглаживай автоматически.\n"
        "   Допускается творческая конкретика: варианты имен, мест, обстоятельств, сценариев.\n"
        "   Подкрепляй это связкой с таро/нумерологией/астрологией/зодиаком, где уместно.\n"
        "2) Не повторяй список «Карты и позиции» — он уже отправлен отдельным сообщением.\n"
        "3) В толковании: каждая карта — отдельным блоком. Привязывай смысл карты к конкретным аспектам запроса.\n"
        ...
        "   Допускается аккуратная психологическая гипотеза («скорее всего», «похоже», «возможно»), без категоричных утверждений.\n"
        "   Периодически поясняй, почему именно такое значение карты здесь уместно (контекст, позиция, тема).\n"
        "   Формат строго:\n"
        "   <b>1) Название карты</b>\n"
        "   1–2 коротких предложения трактовки \n"
        "4) В «Итоге» дай конкретный ответ на запрос пользователя. Можно опираться на знаки зодиака, карты, астрологию, нумерологию. Можно "
        "Итог по смыслу важнее и длиннее, чем блоки по картам.\n"
        "5) Если вопрос формата да/нет — в блоке «Главное» дай явный вероятностный ответ: «Скорее да» / «Скорее нет» и короткую причину.\n"
        "7) В конце с мягкой эмоцией закончи расклад и по дружески предложи 3 примера вопросов для следующего расклада.\n"
        "   Эти вопросы должны логично продолжать тему пользователя.\n"
        "8) Структура: короткие абзацы (1–3 строки), логические блоки, аккуратные маркеры (•).\n"
        "9) Эмодзи: 0–3 на весь ответ, уместно и без перегруза.\n"
        "10) Эмоциональные реплики допустимы 1–2 раза на ответ (например: «Ого, интересный расклад…», «Вау, это сильный знак…»), но без театральности.\n"
        f"11) Длина: 1 карта = коротко; 2-7 карт = немного подробнее. (Сейчас карт: {n_cards}).\n"
        "12) Если в вопросе есть даты/возраст/сроки — используй их как основу для нумерологии/астрологии в толковании.\n"
        "\nФормат ответа:\n"
        "✨ <b>Главное</b> (2–3 строки)\n"
        "🔍 <b>Толкование</b> (по карте, коротко)\n"
        "🧭 <b>Итог</b> (3–5 предложений, конкретно)\n"
        "❓ <b>Мягкий переход в примеры вопросов для следующего расклада</b> (3 пункта)\n"
    )
